...
const StockHistoty = require("../models/stockHistory")(sequelize, DataTypes);
const checkAuth = require("../middlewares/checkAuth");
const checkAuthAdmin = require("../middlewares/checkAuthAdmin");
const router = new Router();

router.get("/", async (req, res, next) => {
  const stocks = await StockHistoty.findAll({
    where: req.query,
  });
  res.json(stocks);
});
...
router.get("/:id", async (req, res, next) => {
  try {
    const stock = await StockHistoty.findByPk(parseInt(req.params.id));
    if (stock) {
      res.json(stock);
    } else {
      res.sendStatus(404);
...
router.post("/", async (req, res, next) => {
  try {
    const stock = await StockHistoty.create(req.body);
