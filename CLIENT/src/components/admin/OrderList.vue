<template>
    <div class="p-6 bg-gray-100 min-h-screen">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold color-dark">Liste des Commandes</h1>
        </div>
        <div class="mb-4">
            <input v-model="searchQuery" type="text" placeholder="Rechercher des commandes..."
            ...
            <table class="min-w-full bg-white">
                ...
                <tbody class="text-gray-600 text-sm font-light">
                    ...
                    <tr v-else v-for="order in paginatedOrders" :key="order.id"
                        ...
                        <td class="py-3 px-6 text-left">{{ order.account.firstName }} {{ order.account.lastName }}</td>
                        <td class="py-3 px-6 text-left">{{ order.total_price.toFixed(2) }} €</td>
                        <td class="py-3 px-6 text-left">{{ new Date(order.order_date).toLocaleDateString() }}</td>
                        <td class="py-3 px-6 text-center">{{ displayOrderStatus(order.order_status) }}</td>
...
const filteredOrders = computed(() => {
    return orders.value.filter(order =>
        order.account.firstName.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
        order.account.lastName.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
        order.id.toString().includes(searchQuery.value) ||
        order.total_price.toString().includes(searchQuery.value) ||
        new Date(order.order_date).toLocaleDateString().includes(searchQuery.value)
    );
});
